...
class ArticleAction extends CommonAction {
   function _initialize(){
    ...
    public function index() {
		...
		$catenews=D("Category")->where(' ch=10 ')->order('ordernum desc')->select();
	  ...
	  function read() {
		...
		$catenews=D("Category")->where(' ch=10 ')->select();
        $this->assign("catenews",$catenews);
		...
		$artersql='';
		if($strcatid!=''){
		$frontsql="ordernum>=".$vo['ordernum']." and id<>".$id."  and ch=10 and catid=".$strcatid."";
		$artersql="ordernum<=".$vo['ordernum']."  and id<>".$id." and ch=10  and catid=".$strcatid."";
			}else{
		$frontsql="ordernum>=".$vo['ordernum']." and id<>".$id." and ch=10 ";
		$artersql="ordernum<=".$vo['ordernum']."  and id<>".$id." and  ch=10 ";	
				}
    ...
    public function huodong() {
		...
		$catenews=D("Category")->where(' ch=2 ')->order('ordernum desc')->select();
        $this->assign("catenews",$catenews);
        $this->assign("seo",$seo);
        $map = $this->_search();
        if (method_exists($this, '_filter')) {
   ...
   public function cases() {
		...
		$catenews=D("Category")->where(' ch=5 ')->order('ordernum desc')->select();
        $this->assign("catenews",$catenews);
        $this->assign("seo",$seo);
        $map = $this->_search();
        if (method_exists($this, '_filter')) {
  ...
  public function pruducts() {
        ...
        if(!empty($_GET['catid'])){
		 ...
		 $catpro=M('category')->where('ch=3  and pid =0  and mid=1')->order('ordernum desc')->select();
			foreach ($catpro as $key => $val) {
                if (is_array($val)) {
					 $catpro[$key]['sublist']=M('category')->where('pid='.$catpro[$key]["id"].' and ch=3')->order('ordernum desc')->select();
                 }
            }
