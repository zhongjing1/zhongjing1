...
<div id="menu">
    <div class="content_title">
        <ul id="nav">
            <li class="cur"><a href="<{site_url("<{$index_dir}>/admin")}>" >公司报名</a></li>
    ...
    <!--预约用户查询-->
    <div>
        <form action="<{site_url("<{$index_dir}>/admin")}>" method="post" enctype="multipart/form-data" name="search_text" style="float: left;">
            <table class="table_search">
                <tr>
                    <td>
                        <input type="text" class="input100" name="person" placeholder="项目负责人" value="<{$person}>"/>
    ...
    <form action="<{site_url("<{$index_dir}>/admin_batch")}>" method="post" enctype="multipart/form-data" name="search_text" id="search_text">
        <table class="tablebg">
            ...
            <tr id="tr_<{$key.id}>">
                ...
                <td><{$key.companyintro}></td>
                <td><{$key.projectintro}></td>
                <td><{$key.jobneed}></td>
                <td align="center"><{$key.person}></td>
...
<script language="javascript">
    ...
    function get_delete(id){
        if(confirm('您确定要删除所选内容吗?\n\n注意：删除后不可恢复\！')){
            $.ajax({
                type:"POST",
                url: "<{site_url("<{$index_dir}>/admin_delete")}>",
                data:"id="+id,
                dataType:"json",
