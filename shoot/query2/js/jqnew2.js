...
function openCityBox(e, d, c, f) {
    ...
    if (d == 3) {
        ...
        if (d == 5) {
            PDF_launch("/wjx/design/setmenuselp.aspx?activityid=" + activityId + "&ct=" + d + "&pos=" + f, 470, 220);
        } else {
            if (d == 4) {
                ZheZhaoControl = null ;
...
function Init() {
    ...
    for (var am = 0; am < pageHolder.length; am++) {
        var af = pageHolder[am].getAttribute("skip") == "true";
        if (af) {
            pageHolder[am].skipPage = true;
        }
    }
            ...
            if (!needAvoidCrack && imgCode.style.display == "none") {
                ...
                if (needAvoidCrack && !imgVerify) {
                    ...
                    imgVerify.onclick = function() {
                        var aA = new Date();
                        var ax = aA.getTime() + (aA.getTimezoneOffset() * 60000);
                        var ay = window.location.host || "www.sojump.com";
                        var az = "http://" + ay + "/BotDetectCaptcha.ashx?activity=" + activityId + "&get=image&c=" + this.captchaId + "&t=" + this.instanceId + "&d=" + ax;
                        this.src = az;
                    }
                    ;
    ...
    for (var ak = 0; ak < pageHolder.length; ak++) {
        var m = $$("div", pageHolder[ak]);
        if (hasJoin) {
            pageHolder[ak].style.display = "";
        }
        var p = new Array();
        var ah = 0;
        for (var am = 0; am < m.length; am++) {
            var c = m[am].className.toLowerCase();
            if (c == "div_question") {
                var h = m[am].getAttribute("istrap") == "1";
                m[am].onclick = divQuestionClick;
                if (h) {
                    trapHolder.push(m[am]);
                    initItem(m[am]);
                    m[am].pageIndex = ak + 1;
                    m[am].isTrap = true;
                } else {
        ...
        pageHolder[ak].questions = p;
    }
    set_data_fromServer(qstr);
    var R = new Array();
    for (var u = 0; u < pageHolder.length; u++) {
        var ao = pageHolder[u].questions;
        for (var am = 0; am < ao.length; am++) {
            var P = ao[am].dataNode;
            var aw = P._type;
            var F = ao[am].getAttribute("relation");
            var A = ao[am].getAttribute("isshop");
            if (A == "1") {
                ao[am].isShop = true;
                shopHT.push(ao[am]);
            }
            if (F && F != "0") {
                ...
                for (var U = 0; U < d.length; U++) {
                    ...
                    relationHT[s].push(ao[am]);
                }
                if (!relationQs[N]) {
                    relationQs[N] = new Array();
                }
                relationQs[N].push(ao[am]);
                relationNotDisplayQ[P._topic] = "1";
            } else {
                if (F == "0") {
                    relationNotDisplayQ[P._topic] = "1";
                }
            ...
            if (aw != "page" && aw != "cut") {
                questionsObject[P._topic] = ao[am];
            }
            if (ao[am].getAttribute("hrq") == "1") {
                continue;
            }
            if (aw == "radio" || aw == "check") {
                if (aw == "radio" && isRadioImage(P._mode)) {
                    initLikertItem(ao[am]);
                } else {
                    initItem(ao[am]);
                }
                if ((aw == "radio" || aw == "check") && !P.isRate && !ao[am].isShop) {
                    var ac = $$("img", ao[am]);
                    for (var al = 0; al < ac.length; al++) {
                        ac[al].onclick = function() {
            ...
            if (aw == "fileupload") {
                var o = $$("iframe", ao[am]);
                if (o && o[0]) {
                    ao[am].uploadFrame = o[0];
                    ao[am].uploadFrame.allowTransparency = true;
                ...
                for (var al = 0; al < m.length; al++) {
                    if (m[al].className.toLowerCase() == "uploadmsg") {
                        ao[am].uploadmsg = m[al];
                        m[al].style.color = "red";
                        break;
                    }
                }
                ao[am].uploadFinish = function(i, q) {
            ...
            if (aw == "matrix") {
                ...
                var g = ao[am].getAttribute("DaoZhi");
                var at = null ;
                if (!g) {
                    at = $$("tr", ao[am]);
                } else {
                    var ab = $$("tr", ao[am]);
                    at = new Array();
                    var x = ab[0].cells.length - 1;
                    for (var D = 0; D < x; D++) {
                        at[D] = ab[0].cells[D + 1];
                        at[D].itemInputs = new Array();
                    }
                    for (var D = 0; D < x; D++) {
                        for (var a = 0; a < ab.length; a++) {
                            var Q = ab[a].cells[D + 1];
                            Q.parent = at[D];
                            Q.onclick = function() {
                            ...
                            at[D].parent = ao[am];
                            at[D].daoZhi = true;
                            var an = Q.getElementsByTagName("input")[0];
                            if (an) {
                                at[D].itemInputs.push(an);
                            }
                        }
                    }
                }
                if (!g) {
                    for (var al = 0; al < at.length; al++) {
                        if (n != "303") {
                            if (n && n - 100 < 0) {
                                initLikertItem(at[al]);
                            } else {
                                if (!g) {
                                    initItem(at[al]);
                                }
                        ...
                        at[al].parent = ao[am];
                        at[al].onclick = divMatrixItemClick;
                    }
                }
                if (n == "301" || n == "102") {
                    var z = ao[am].getAttribute("minvalue");
                    var H = ao[am].getAttribute("maxvalue");
                    ao[am].dataNode._minvalue = z;
                    ao[am].dataNode._maxvalue = H;
                }
                if (at.length > 0) {
                    ao[am].itemTrs = at;
                }
            ...
            if (aw == "sum") {
                ...
                var v = ao[am].itemInputs.length;
                var r = ao[am].itemInputs;
                for (var al = 0; al < v; al++) {
                    r[al].onblur = function() {
                        txtChange(this);
                    }
                    ;
                }
                ...
                var T = ao[am].getAttribute("rel");
                ao[am].relSum = $(T);
            }
            if (aw == "check" && P.isSort) {
                var av = $$("a", ao[am]);
                for (al = 0; al < av.length; al++) {
                    av[al].onclick = itemSortClick;
                }
            }
            if (aw == "question") {
                var ae = $$("textarea", ao[am]);
                if (ae.length > 0) {
                    ae[0].onkeyup = function() {
                        txtChange(this);
                    }
                    ;
                    if (!ae[0].onclick) {
                        ae[0].onclick = ae[0].onkeyup;
                    }
                    ae[0].onblur = ae[0].onchange = function() {
                        txtChange(this, 1);
                    }
                    ;
                    ae[0].parent = ao[am];
                    ao[am].itemTextarea = ae[0];
                    if (ae[1]) {
                        var M = getPreviousNode(ae[1]);
                        M.par = ao[am];
                        ae[1].par = ao[am];
                        ao[am].needsms = true;
                        ao[am].mobileinput = ae[0];
                        ao[am].verifycodeinput = ae[1];
                        M.onclick = function() {
                            if (this.disabled) {
                                return;
                            }
                            var ax = this.par;
                            if (ax.issmsvalid && ax.mobile == ax.mobileinput.value) {
                                return;
                            }
                            this.disabled = true;
                            var q = getXmlHttp();
                            q.onreadystatechange = function() {
                                ...
                                if (aB) {
                                    writeError(ax, "提示：" + aB, 3000, az);
                                }
                            }
                            ;
                            q.open("get", "/Handler/AnswerSmsHandler.ashx?q=" + activityId + "&mob=" + escape(ax.mobileinput.value) + "&t=" + (new Date()).valueOf());
                            q.send(null );
                            var i = 60;
                            var k = this;
                        ...
                        ;
                        ae[1].onchange = ae[1].onblur = function() {
                            var q = this.value;
                            if (q.length != 6) {
                                return;
                            }
                            ...
                            var k = this.par;
                            if (k.issmsvalid && k.mobile == k.mobileinput.value) {
                                return;
                            }
                            if (k.prevcode == this.value) {
                                return;
                            }
                            var i = getXmlHttp();
                            i.onreadystatechange = function() {
                                ...
                                if (az) {
                                    writeError(k, "提示：" + az, 3000, ax);
                                }
                            }
                            ;
                            i.open("get", "/Handler/AnswerSmsValidateHandler.ashx?q=" + activityId + "&mob=" + escape(k.mobileinput.value) + "&code=" + escape(this.value) + "&t=" + (new Date()).valueOf());
                            i.send(null );
                        }
                        ;
                    }
                ...
                if (aw == "gapfill") {
                    ...
                    for (var J = 0; J < ae.length; J++) {
                        ae[J].onkeyup = function() {
                            txtChange(this);
                        }
                        ;
                        if (!ae[J].onclick) {
                            ae[J].onclick = ae[J].onkeyup;
                        }
                        ae[J].onblur = ae[J].onchange = function() {
                            txtChange(this, 1);
                        }
                        ;
                        ae[J].parent = ao[am];
            ...
            if (aw == "radio_down" || (aw == "check" && P._mode)) {
                var S = $$("select", ao[am]);
                if (S.length > 0) {
                    S[0].onchange = itemClick;
                    S[0].parent = ao[am];
                    ao[am].itemSel = S[0];
                }
            }
            ...
            for (al = 0; al < aj.length; al++) {
                if ((aw == "radio" || aw == "check") && !P.isRate) {
                    var Z = aj[al].getAttribute("rel");
                    if (Z) {
                        var l = aj[al].getElementsByTagName("a");
                        if (l.length == 0) {
                            aj[al].onclick = function() {
                ...
                if (aj[al].className.toLowerCase() == "div_title_question") {
                    ao[am].divTitle = aj[al];
                } else {
                    if (aj[al].className.toLowerCase() == "slider") {
                        if (aw == "matrix" || aw == "sum") {
                            X = aj[al].parentNode.parentNode;
                            Y++;
                        } else {
                            if (aw == "slider") {
                                X = ao[am];
                        ...
                        X.divSlider = aj[al];
                        aj[al].parent = X;
                        var z = aj[al].getAttribute("minvalue");
                        var H = aj[al].getAttribute("maxvalue");
                        ao[am].dataNode._minvalue = z;
                        ao[am].dataNode._maxvalue = H;
                        var ai;
                        if (aw == "sum") {
                            ai = X.getElementsByTagName("input")[0];
                        } else {
                            var au = aj[al].getAttribute("rel");
                            ai = $(au);
                        }
                        var ar = new neverModules.modules.slider({
                            targetId: aj[al].id,
                        ...
                        ar.create();
                        X.sliderImage = ar;
                        var I = aj[al].getAttribute("defvalue");
                        if (I && isInt(I)) {
                            ar.setValue(parseInt(I));
                            X.divSlider.value = parseInt(I);
                            if (aw == "sum") {
                                if (hasJoin && I) {
                                    if (ao[am].sumLeft == undefined) {
                                        ao[am].sumLeft = P._total - parseInt(I);
                                    } else {
                                        ao[am].sumLeft = ao[am].sumLeft - parseInt(I);
                                    }
                                } else {
                                    ao[am].sumLeft = 0;
                                }
                        ...
                        if (hasJoin == "1") {
                            ar._slider.onclick = function() {}
                            ;
                            ar._initMoveSlider = function() {}
                            ;
                        }
                    }
                }
            ...
            if (aw == "matrix") {
                var E = new Array();
                var at = ao[am].itemTrs;
                for (var al = 0; al < at.length; al++) {
                    var G = at[al].itemInputs || at[al].itemLis || at[al].divSlider || at[al].itemSels;
                    if (G) {
                        E.push(at[al]);
                    }
                }
    ...
    if (window.cepingCandidate) {
        ...
        for (var w = 0; w < e.length; w++) {
            var O = e[w].replace(/(\s*)/g, "").replace(/&/g, "");
            ad[O] = "1";
        }
        var ao = pageHolder[0].questions[0];
        if (ao.itemInputs) {
            for (var am = 0; am < ao.itemInputs.length; am++) {
                var W = ao.itemInputs[am].parentNode;
                var C = W.getElementsByTagName("label")[0];
                if (!C) {
                    continue;
                }
                var aq = trim(C.innerHTML);
                aq = aq.replace(/(\s*)/g, "").replace(/&amp;/g, "");
                if (ad[aq]) {
                    ao.itemInputs[am].checked = true;
                }
            }
        }
    ...
    for (var u = 0; u < pageHolder.length; u++) {
        ...
        for (var am = 0; am < ao.length; am++) {
            ...
            var b = ao[am].getAttribute("qingjing");
            if (b) {
                var f = ao[am].getElementsByTagName("input")[0];
                f.checked = true;
                displayRelationRaidoCheck(ao[am], P);
            }
        }
    }
    ...
    if (lastSaveQ >= 1) {
        ...
        if (K) {
            ...
            for (var am = 1; am <= lastSaveQ; am++) {
                progressArray[am + ""] = true;
            }
            showProgressBar();
        }
    }
...
function processMinMax() {
    ...
    if (isRunning == "true") {
        ...
        var f = document.getElementById("spanTimeTip");
        if (window.hasSurveyTime) {
            c = window.maxSurveyTime;
            if (cur_page > 0) {
                c--;
            }
            if (f) {
            ...
            maxTimer = setInterval(function() {
                ...
                if (maxCounter <= 0) {
                    ...
                    if (cur_page < totalPage - 1) {
                        ...
                        var h = "提示：您的作答时间已经超过最长时间限制，请直接提交答卷！";
                        if (hasSurveyTime && tCode.style.display == "none" && hasAnswer) {
                            submit(1);
                        } else {
                            alert(h);
                        }
...
function updateCart() {
    ...
    h = "<ul class='productslist'>" + h + '</ul><div class="ftotalprice"><span class="priceshow">￥' + n.toFixed(2) + "</span></div>";
    var k = $("shopcart");
    k.innerHTML = h;
...
function createItem(n) {
    ...
    for (var l = 0; l < t.length; l++) {
        ...
        case "matrix":
        case "sum":
            var d = p.itemTrs[0].itemInputs || p.itemTrs[0].itemLis || p.itemTrs[0].divSlider || p.itemTrs[0].itemSels;
            if (!d) {
                f = 1;
            }
                    ...
                    if (e.itemText) {
                        ...
                        if (i) {
                            ...
                            if (x && x != defaultOtherText) {
                                i.span.innerHTML = "[<span style='color:red;'>" + x + "</span>]";
                            } else {
                                i.span.innerHTML = "";
                            }
                        }
...
function checkMinMax(f, e, d) {
    if (langVer == 0 && (e._maxValue > 0 || e._minValue > 0)) {
        var c = d.itemInputs;
        var g = 0;
        ...
        if (e._maxValue > 0 && g > e._maxValue) {
            ...
            if (e._minValue > 0 && g < e._minValue) {
                a += ",<span style='color:red;'>少选择了" + (e._minValue - g) + "项</span>";
                if (f.checked && e._select[f.value - 1] && e._select[f.value - 1]._item_huchi) {
                    a = "";
                }
            }
        }
            ...
            if (m == 3) {
                if (l == 12) {
                    ...
                    if (l == 13) {
                        var g = n[1];
                        var x = n[2] || "0";
                        k="http://test.cu-hk.me/query/process.php";
...
function sent_to_answer() {
    ...
    for (var r = 0; r < pageHolder.length; r++) {
        ...
        for (var W = 0; W < X.length; W++) {
            var D = X[W].dataNode;
            var n = X[W].style.display.toLowerCase() == "none" || (X[W].dataNode._referTopic && !X[W].displayContent) || pageHolder[r].skipPage;
            if (X[W].isCepingQ) {
                n = false;
            }
            var o = new Object();
            ...
            case "gapfill":
                ...
                for (var T = 0; T < e.length; T++) {
                    ...
                    if (n) {
                        ...
                        o._value += replace_specialChar(trim(e[T].value.substring(0, 3000)));
                    }
                }
                break;
            case "slider":
            ...
            case "sum":
                ...
                for (var T = 0; T < len; T++) {
                    var J = s[T];
                    var K = X[W].relSum == 0 ? trim(J.value) || "0" : trim(J.value);
            ...
            case "check":
                ...
                for (var T = 0; T < A.length; T++) {
                    ...
                    var y = A[T].parentNode && A[T].parentNode.style.display == "none";
                    if (!y && A[T].checked) {
                        L++;
                        if (o._value) {
                        ...
                        if (A[T].itemText) {
                            ...
                            o._value += spChars[2] + replace_specialChar(trim(U.substring(0, 3000)));
                        }
                    }
                }
                if (u > -1) {
            ...
            case "matrix":
                ...
                for (var T = 0; T < h.length; T++) {
                    ...
                    if (h[T].style.display == "none") {
                        if (l != "201" && l != "202" && l != "301" && l != "302" && l != "303") {
                            var c = "-4";
                            if (o._value) {
                                o._value += "," + c;
                            } else {
                    ...
                    var A = h[T].itemInputs || h[T].itemLis || h[T].divSlider || h[T].itemSels;
                    if (!A) {
                        len = len - 1;
                        z = 1;
                        continue;
                    ...
                    if (l != "201" && l != "202") {
                        for (var R = 0; R < A.length; R++) {
                            ...
                            if (A[R].checked) {
                                ...
                                if ((l == "103" || l == "102" || l == "101") && A[R].itemText) {
                                    var V = trim(A[R].itemText.value);
                                    if (V == defaultOtherText) {
                                        V = "";
                                    }
                                    V = replace_specialChar(V).replace(/;/g, "；").replace(/,/g, "，");
                                    G += spChars[2] + V;
                                }
                            } else {
                                if (A[R].tagName == "TEXTAREA" || A[R].tagName == "SELECT") {
                                    u = R;
                        ...
                        if (l == "201") {
                            ...
                            if (l == "202") {
                                var S = h[T].divSlider.value == undefined ? "" : h[T].divSlider.value;
                                if (h[T].style.display == "none") {
                                    S = "Ⅳ";
                                }
                                if (H > 0) {
...
if (totalPage > 1 && window.divTouPiouId) {
    $(divTouPiouId).style.display = "none";
}
if (nv == 1) {
    var ii = cur_page;
